...
from PySide6.QtWidgets import (
    ...
    QVBoxLayout, QHBoxLayout, QLineEdit, QLabel, QComboBox, QDialog, QAbstractItemView
)
from PySide6.QtGui import QFont
from PySide6.QtCore import Qt, QRect, Signal
from .modify_ui import Ui_dataloader  # 导入自动生成的UI类
from tools.flowlayout import FlowLayout, KeywordLabel  # 导入小标签功能
...
from tools.database_manager import DatabaseManager  # 导入数据库管理功能
from show_modify_page.show_modify_page import ShowModifyPage  # 导入修改页面
...
class ModifyPage(QWidget):
    ...
    def __init__(self, db_manager=None, parent=None):
        ...
        self.main_layout.addWidget(self.ui.label)
        # 将原有UI布局添加到主布局中
        self.main_layout.addWidget(self.ui.scrollArea)
        ...
        # 动态添加显示已选择数量的标签
        self.selection_label = QLabel("目前已选择数量：0", self.ui.scrollAreaWidgetContents)
        self.selection_label.setGeometry(QRect(10, 520, 200, 31))  # 根据实际布局调整位置
        font = QFont()
    ...
    def setup_keyword_container(self):
        """通过代码设置关键字标签区域，避免修改 .ui 文件"""
        # 创建关键词容器
        self.keywords_widget = QWidget(self.ui.scrollAreaWidgetContents)
        self.keywords_layout = FlowLayout(self.keywords_widget)
        self.keywords_layout.setSpacing(5)

        # 设置关键词容器的位置和大小
        # 假设搜索框是 self.ui.lineEdit, 获取其几何信息
        search_box_geometry = self.ui.lineEdit.geometry()
        x = search_box_geometry.x()
        y = search_box_geometry.y() + search_box_geometry.height() + 10  # 搜索框下方 10 像素
        width = self.ui.scrollAreaWidgetContents.width() - 2 * x
        height = 30  # 根据需要调整高度,30

        self.keywords_widget.setGeometry(QRect(x, y, width, height))
        self.keywords_widget.setObjectName("keywordContainer")

        # 设置 FlowLayout 到 keywords_widget
    ...
    def add_keyword_label(self, keyword):
        ...
        label.remove_clicked.connect(self.remove_keyword)  # 连接信号
    ...
    def remove_keyword(self, keyword):
        ...
        if keyword in self.keywords:
            ...
            logging.info(f"移除分类：{keyword}")
        else:
            logging.warning(f"尝试移除不存在的分类：{keyword}")
    ...
    def handle_add_keyword(self):
        """处理添加关键词按钮点击"""
        adder = AddKeyword(self, self.ui.lineEdit, self.keywords, self.keywords_layout)
        adder.execute()
    ...
    def handle_undo_operation(self):
        """处理撤销操作按钮点击"""
        undoer = UndoOperation(self, self.db_manager, self.ui.tableWidget, self.db_name)
        undoer.execute()
        self.load_existing_records()

    def load_categories_into_combobox(self):
        """加载分类到组合框，包括没有相关记录的分类"""
        current_selection = self.ui.comboBox.currentText()
        self.ui.comboBox.blockSignals(True)  # 阻塞信号以防止触发过滤
        self.ui.comboBox.clear()
        self.ui.comboBox.addItem("全部")
        categories = self.db_manager.get_categories()
        self.ui.comboBox.addItems(categories)
        # 保持之前的选择，如果存在
        index = self.ui.comboBox.findText(current_selection)
        if index != -1:
